...
export class PMIRepositoryImpl implements PMIRepository {
    ...
    async getAll(): Promise<ArticleEntity[]> {
        ...
        const result = await request.query<any[]>(`SELECT ARKTCODART, ARKTCOMART, ARCTLIB01, ARCTLIB02, ARCTCODFAM,  dbo.get_libelle_tt_doc_optimized_secure(ARKTCODART, ARKTCOMART) as 'label', ARCNSTOPHY, ARCNDISPO, ARCNPUACH2 FROM ARTICLE WHERE ARKTSOC = '100'`);
